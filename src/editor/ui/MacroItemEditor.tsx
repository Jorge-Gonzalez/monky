  ...
  return (
    <div className="card padding-md ground rule corner-md ruled">
      ...
      <div className="button-group horizontal inline" style={{ marginTop: 'var(--spacing-md)' }}>
        <button className="btn-link ink-accent font-sm undecorated hover:underlined" onClick={() => onEdit(macro)}>{t('macroItemEditor.edit')}</button>
        <button className="btn-link-danger ink-fail font-sm undecorated hover:underlined" onClick={() => onDelete(String(macro.id))}>{t('macroItemEditor.delete')}</button>
      </div>
    </div>
  )
}
