...
export function MacroSearchFooter({ count, isCommandMode, hasSelection }: MacroSearchFooterProps) {
  ...
  return (
    <div
      ...
      className="horizontal padding-block-sm padding-inline-xl justify-between
        ground ink-soft font-sm"
    >
      <div
        data-component="search-footer-count"
        className="rigid
      ...
      <div
        data-component="search-footer-hints"
        className="horizontal gap-md align-center"
      >
        {showShortcuts &&
          (isCommandMode ? (
            <>
              <ShortcutHint
                keys={['↑', '↓']}
                label={t('modalSearch.footer.navigate')}
              />
              <ShortcutHint
                keys={['\u2009↵\u2009']}
                label={t('modalSearch.footer.run')}
              />
              <ShortcutHint
                keys={['Esc']}
                label={t('modalSearch.footer.close')}
                last
              />
            </>
          ) : (
            <>
              <ShortcutHint
                keys={[':']}
                label={t('modalSearch.footer.commandsLabel')}
              />
              <ShortcutHint
                keys={['↑', '↓']}
                label={t('modalSearch.footer.navigate')}
              ...
              <ShortcutHint
                keys={['\u2009↵\u2009']}
                label={t('modalSearch.footer.select')}
              />
              {hasSelection && (
                <ShortcutHint
                  keys={['Tab']}
                  label={t('modalSearch.footer.edit')}
                />
              )}
              <ShortcutHint
                keys={['Esc']}
        ...
        <button
          ...
          aria-label={t(
            showShortcuts ? 'modalSearch.footer.hideShortcuts' : 'modalSearch.footer.showShortcuts'
          )}
        ...
        >
          ...
          >
            ...
            />
            <path
              d="M6.7 6.1a1.5 1.5 0 1 1 2.6 1c-.8.7-1.3 1-1.3 2"
              stroke="currentColor"
              strokeLinecap="round"
            />
            <circle
