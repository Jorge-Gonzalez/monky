...
describe('MacroSearchResults', () => {
  ...
  describe('placeholder text', () => {
    const textOf = (container: Element) => container.querySelector('[data-component="search-item-text"]')!

    it('fades the braces and leaves the label plain', () => {
      const { container } = render(
        <MacroSearchResults
          {...baseProps}
          macros={[{ id: 9, command: '/greet', text: 'Hi {{name}}!' }]}
        />
      )
    ...
    it('fades every placeholder when there is more than one', () => {
      const { container } = render(
        <MacroSearchResults
          {...baseProps}
          macros={[{ id: 9, command: '/m', text: '{{a}} and {{b}}' }]}
        />
      )
      const text = textOf(container)
