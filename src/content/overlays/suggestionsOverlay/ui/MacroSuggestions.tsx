      ...
      <div ref={listRef} role="listbox" className="macro-suggestions-commands-list horizontal padding-tight gap-tight rule-soft ruled-bottom">
        {visibleMacros.map((macro, index) => (
          <button
            ...
            className="macro-suggestions-command-item compressible min-width-none ground-subtle ink rule-soft ruled corner-md hidden truncate font-sm selectable hover:ground-defined hover:rule selected:ground-defined selected:ink-accent selected:rule-accent"
            onMouseDown={(e) => {
              e.preventDefault();
              onSelectMacro(macro);
            }}
